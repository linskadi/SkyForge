...
/** 计算波形统计信息 */
export function computeStats(input: number[], output: number[]): SimulationStatistics {
  const inMin = input.reduce((a, b) => Math.min(a, b), Infinity);
  const inMax = input.reduce((a, b) => Math.max(a, b), -Infinity);
  const outMin = output.reduce((a, b) => Math.min(a, b), Infinity);
  const outMax = output.reduce((a, b) => Math.max(a, b), -Infinity);
