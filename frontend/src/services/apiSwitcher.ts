...
import { reactive, readonly, toRefs, type DeepReadonly } from "vue";
...
export interface ApiInterface {
  ...
  generateReport(pipelineResult: any): Promise<ReportResult>;
  downloadReport(): string;
  compose(compA: any, compB: any, connection: string): Promise<ComposeResult>;
  checkCompatibility(
    contractA: string,
    contractB: string,
    connection: string,
...
export function setUseRealAPI(value: boolean, persist = true): void {
  ...
  console.info(`[apiSwitcher] API 模式已切换为：${value ? "真实 API" : "Mock"}`);
}
...
/** 类型导出，便于组件使用 */
export type { GenerateResult, ComposeResult, ReportResult, MisraRule } from "./mockApi";
