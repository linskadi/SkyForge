"""Day 3 测试：数字孪生仿真引擎（虚拟传感器 + GCC 编译运行 + 故障注入 + 契约断言 core dump）。
...
from app.core.digital_twin.virtual_mcu import RunResult, VirtualMCU
...
class TestMockMode(unittest.TestCase):
    """mock 模式（无 GCC）优雅降级测试。"""

    def test_mock_mode_compile(self) -> None:
        """无 GCC 时 compile 返回 mock 结果。"""
        mcu = VirtualMCU()
        if mcu.is_gcc_available():
            self.skipTest("GCC 已安装，跳过 mock 模式测试")
        result = mcu.compile(FILTER_CODE)
        self.assertTrue(result.success)
        self.assertTrue(result.used_mock)
    ...
    def test_mock_mode_in_simulation_engine(self) -> None:
        """SimulationEngine 在 mock 模式下也能完成仿真。"""
        mcu = VirtualMCU()
        if mcu.is_gcc_available():
            self.skipTest("GCC 已安装，跳过 mock 模式测试")
        ...
        self.assertTrue(result.passed)
        self.assertTrue(result.compilation["used_mock"])
        self.assertIn("Python 模拟", result.terminal_log)
