...
class CCodeAnalyzer:
    ...
    def has_range_check(self, var_name: str, min_val: str = None, max_val: str = None) -> list[dict]:
        """检查是否存在范围检查。"""
        results = []
        for stmt in self.if_statements:
            condition = stmt["condition"]
...
class CppcheckVerifier:
    ...
    def verify(self, code: str, contract_type: str = None) -> dict[str, Any]:
        ...
        Args:
            code: C 代码字符串
            contract_type: 契约类型 (precondition/postcondition/invariant/fault_handling)
    ...
    def _filter_violations(self, violations: list[dict], contract_type: str) -> list[dict]:
        """根据契约类型筛选相关违规。"""
...
class SemanticChecker:
    ...
    def check_precondition(self, expr: str, cid: str, index: int) -> CheckItem:
        ...
        # 计算置信度
        confidence = self._calculate_precondition_confidence(evidence, div_issues, uninit_vars, buffer_issues)

        # 判断是否通过
        passed = confidence >= ConfidenceLevel.MODERATE and not div_issues and not uninit_vars
    ...
    def check_postcondition(self, expr: str, cid: str, index: int) -> CheckItem:
        ...
        if "=" in expr or "return" in expr:
            ...
            if self.analyzer.return_statements:
                evidence.append(f"检测到 {len(self.analyzer.return_statements)} 个返回语句")

        # 2. 检查输出变量是否被赋值
        output_match = re.search(r"(\w+)\s*(>=|<=|>|<)\s*(-?\d+)", expr)
        if output_match:
            var_name, op, value = output_match.groups()
            assignments = [a for a in self.analyzer.assignments if a["target"] == var_name]
            if assignments:
    ...
    def check_invariant(self, expr: str, cid: str, index: int) -> CheckItem:
        ...
        # 2. 检查 const 变量
        const_vars = [d for d in self.analyzer.variable_declarations if "const" in self.code.splitlines()[d["line"]-1]]
