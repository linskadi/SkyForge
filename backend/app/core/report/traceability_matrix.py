"""追溯矩阵构建器：从 pipeline_result 提取 [REQ-xxx] / [CON-xxx] / 代码行号 / [TST-xxx]，
构建双向追溯表：需求 → 契约 → 代码 → 测试。

数据来源：
- 需求：pipeline_result["structured_reqs"]（列表）或 pipeline_result["requirement"]（单条 dict）
- 契约：pipeline_result["contract"]（YAML 字符串），从 traceability 字段反查 REQ ↔ CON
- 代码：pipeline_result["final_code"] 或 ["code"]，提取含 [REQ-xxx] [MISRA-Rule-x.x] 的注释行号
...
# Tag 正则：[REQ-xxx] / [CON-xxx] / [MISRA-Rule-x.x]
...
class TraceEntry:
    ...
    Attributes:
        req_id: 需求 Tag（如 REQ-001）。
        req_desc: 需求描述（自然语言）。
        contract_id: 契约 Tag（如 CON-001），从契约 YAML traceability 反查。
        code_line: 代码中含 [REQ-xxx] [MISRA-Rule-x.x] 注释的行号（1-based；0 表示无匹配）。
        code_snippet: 该行代码文本（去前后空白）。
        test_id: 测试 Tag（如 TST-001），由契约检查项 + 仿真结果合成。
...
def build_matrix(pipeline_result: dict[str, Any]) -> list[TraceEntry]:
    """构建追溯矩阵：需求 → 契约 → 代码 → 测试。

    Args:
        pipeline_result: 全流程结果字典，至少包含 requirement / contract / final_code
            （或 code）/ contract_check_result / simulation_result 字段。
    ...
    logger.info(f"TraceabilityMatrix:构建完成: {len(entries)} 行追溯条目")
    return entries
