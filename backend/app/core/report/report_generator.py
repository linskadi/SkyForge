"""DO-178C 合规报告生成器：基于 pipeline_result 渲染 HTML 报告（内嵌 CSS，支持浏览器打印 PDF）。
...
<!-- ========== 2. 需求追溯矩阵 ========== -->
<h2>1. 需求追溯矩阵</h2>
<p>下表展示需求 [REQ-xxx] → 契约 [CON-xxx] → 代码行号 → 测试 [TST-xxx] 的双向追溯链：</p>
{% if trace_matrix %}
<table>
  <thead>
    <tr>
      <th style="width: 10%;">需求 Tag</th>
      <th style="width: 30%;">需求描述</th>
      <th style="width: 10%;">契约 Tag</th>
      <th style="width: 8%;">代码行号</th>
      <th style="width: 26%;">代码片段</th>
      <th style="width: 8%;">测试 Tag</th>
      <th style="width: 8%;">测试结果</th>
    </tr>
  </thead>
  <tbody>
  {% for e in trace_matrix %}
    <tr>
      ...
      <td>{{ e.req_desc }}</td>
      <td>{% if e.contract_id %}<span class="badge badge-con">{{ e.contract_id }}</span>{% endif %}</td>
      <td>{% if e.code_line %}L{{ e.code_line }}{% else %}-{% endif %}</td>
      <td><code>{{ e.code_snippet }}</code></td>
      <td>{% if e.test_id %}<span class="badge badge-tst">{{ e.test_id }}</span>{% else %}-{% endif %}</td>
      <td>
...
<table>
  ...
  {% for item in contract_check_result.preconditions %}
    <tr>
      ...
      <td>{{ item.desc }}</td>
      <td>{% if item.passed %}<span class="status-pass">通过</span>{% else %}<span class="status-fail">失败</span>{% endif %}</td>
      <td>{{ item.detail }}</td>
    </tr>
  {% endfor %}
  </tbody>
...
<div class="summary-card">
  ...
  <div class="stat-grid">
    <div class="stat"><div class="num">{{ cppcheck_count }}</div><div class="label">初次违规</div></div>
    <div class="stat"><div class="num">{{ repair_rounds }}</div><div class="label">修复轮次</div></div>
    <div class="stat"><div class="num">{{ total_actions }}</div><div class="label">修复动作</div></div>
    <div class="stat"><div class="num">{{ final_violations_count }}</div><div class="label">残留违规</div></div>
  </div>
</div>
...
<div class="summary-card">
  <div class="stat-grid">
    <div class="stat"><div class="num">{{ simulation_result.total_steps }}</div><div class="label">仿真步数</div></div>
    <div class="stat"><div class="num">{{ sim_input_min }}</div><div class="label">输入最小</div></div>
    <div class="stat"><div class="num">{{ sim_input_max }}</div><div class="label">输入最大</div></div>
    <div class="stat"><div class="num">{{ sim_output_min }}</div><div class="label">输出最小</div></div>
  </div>
  <div class="stat-grid" style="margin-top: 12px;">
    <div class="stat"><div class="num">{{ sim_output_max }}</div><div class="label">输出最大</div></div>
    <div class="stat"><div class="num">{{ sim_duration_ms }} ms</div><div class="label">运行时长</div></div>
    <div class="stat">
      <div class="num">
...
<p>已注入故障：<strong>{{ simulation_result.fault_type }}</strong>，参数：<code>{{ simulation_result.fault_params }}</code></p>
{% else %}
<p>本次仿真未注入故障。</p>
{% endif %}
...
<table>
  ...
  <tbody>
    <tr>
      <td><span class="badge badge-con">{{ simulation_result.contract_violation.contract_id }}</span></td>
...
<div class="summary-card">
  <div class="stat-grid">
    <div class="stat"><div class="num">{{ obj_pass_count }}</div><div class="label">满足</div></div>
    <div class="stat"><div class="num">{{ obj_partial_count }}</div><div class="label">部分满足</div></div>
    <div class="stat"><div class="num">{{ obj_fail_count }}</div><div class="label">未满足</div></div>
    <div class="stat"><div class="num">{{ objectives | length }}</div><div class="label">总目标数</div></div>
  </div>
</div>
<table>
  <thead>
  ...
  {% for obj in objectives %}
    <tr>
      ...
      <td>
        ...
        {% elif obj.status == "部分满足" %}<span class="badge badge-partial">部分满足</span>
