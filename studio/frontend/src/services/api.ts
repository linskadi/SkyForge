 ...
 */
function transformScadeResponse(raw: RawResponse): ScadeParseResult {
 ...
 */
function transformLLMStatusResponse(raw: RawResponse): LLMStatus {
 ...
 */
function transformModelsResponse(raw: RawResponse): LLMModel[] {
	const list = raw.models ?? raw ?? [];
	return list.map((m: RawResponse) => {
		if (typeof m === "string") {
 ...
 */
function transformPendingResponse(raw: RawResponse): HILApproval[] {
...
export async function repair(code: string): Promise<RepairResult> {
	return withFallback(
		async () => {
			const raw = await postJSON<RawResponse>("/api/repair", {
				code,
				contract: "",
				max_iterations: 3,
				req_id: "REQ-001",
...
export async function getFaultTypes(): Promise<FaultType[]> {
	return withFallback(
		async () => {
			const raw = await getJSON<RawResponse>("/api/fault-types");
			const list = raw.fault_types ?? [];
			return list.map((item: RawResponse) =>
...
export async function checkCompatibility(
	...
	return withFallback(
		async () => {
			...
			return transformCompatibilityResponse(
				raw,
				contractA,
				contractB,
				connection,
...
export async function getLLMStatus(): Promise<LLMStatus> {
	return withFallback(
		async () => {
			const raw = await getJSON<RawResponse>("/api/llm/status");
...
export async function getModels(): Promise<LLMModel[]> {
	return withFallback(
		async () => {
			const raw = await getJSON<RawResponse>("/api/models");
	...
	return withFallback(
		async () => {
			const raw = await getJSON<RawResponse>(
				`/api/misra/search?q=${encodeURIComponent(query)}`,
			);
	...
	return withFallback(
		async () => {
			const raw = await getJSON(
				`/api/misra/rules/${encodeURIComponent(ruleId)}`,
			);
