...
export function buildReport(result: GenerateResult): {
	...
	const summary: ReportSummary = {
		title: dataT(
			"data.report.title",
			`DO-178C 报告 - ${result.contract.component}`,
			{
				component: result.contract.component,
			},
		),
		...
		simulation_summary: simPassed
			? dataT(
					"data.report.simulationPass",
					"数字孪生仿真通过，全部契约断言无违约",
				)
			: dataT(
					"data.report.simulationFail",
					"仿真中发现契约违约，详见仿真报告章节",
				),
	...
	const generatedAtLine = dataT(
		"data.report.generatedAt",
		`生成时间：${new Date(generatedAt).toLocaleString("zh-CN")} ｜ 报告 ID：<code>${escapeHtml(reportId)}</code>`,
		{
			time: new Date(generatedAt).toLocaleString(
				i18n.global.locale.value === "zh-CN" ? "zh-CN" : "en-US",
			),
			id: escapeHtml(reportId),
		},
	);
	const passWord = dataT("data.report.pass", "通过");
	const failWord = dataT("data.report.fail", "违约");
	const itemPassWord = dataT("data.report.pass", "通过");
	const itemFailWord = dataT("data.report.itemFail", "失败");
	const simResultPass = dataT(
		"data.report.simResultPass",
		"✅ 全部契约断言通过",
	);
	const simResultFail = dataT("data.report.simResultFail", "❌ 存在契约违约");
	const sectionTitle = (key: string, fallback: string): string =>
		dataT(key, fallback);
	const statObjectives = dataT(
		"data.report.statObjectives",
		"DO-178C 目标通过率",
	);
	const statTraceability = dataT(
		"data.report.statTraceability",
		"追溯矩阵条目数",
	);
	const statMisra = dataT("data.report.statMisra", "MISRA-C 违规数");
	const statPassRate = dataT("data.report.statPassRate", "总通过率");
...
<body>
  ...
  <div class="summary-card">
    <div class="stat">
      ...
      <div class="stat-label">${statObjectives}</div>
    </div>
    <div class="stat">
      <div class="stat-value">${summary.traceability_entries}</div>
      <div class="stat-label">${statTraceability}</div>
    </div>
    <div class="stat">
      <div class="stat-value">${summary.misra_violations}</div>
      <div class="stat-label">${statMisra}</div>
    </div>
    <div class="stat">
      <div class="stat-value">${Math.round(summary.pass_rate * 100)}%</div>
      <div class="stat-label">${statPassRate}</div>
    </div>
  </div>

  <h2>${sectionTitle("data.report.sectionSummary", "1. 执行摘要")}</h2>
  ...
  <ul>
    ...
    <li>数字孪生仿真：<span class="badge ${simPassed ? "pass" : "fail"}">${simPassed ? passWord : failWord}</span></li>
    <li>MISRA-C 违规数：${misraCount}</li>
    <li>修复迭代轮数：${result.repair_history.length}</li>
  </ul>

  <h2>${sectionTitle("data.report.sectionTraceability", "2. 需求追溯矩阵")}</h2>
  <table>
    <thead>
  ...
  <h2>${sectionTitle("data.report.sectionContract", "3. 契约校验结果")}</h2>
  <table>
    <thead>
    ...
    <tbody>
      ${result.contract_check_result.sections
				.flatMap((s) =>
					s.items.map(
						(it) =>
							`<tr><td><code>${escapeHtml(it.id)}</code></td><td>${escapeHtml(dataT(s.title, s.title))}</td><td><code>${escapeHtml(it.expression)}</code></td><td><span class="badge ${it.passed ? "pass" : "fail"}">${it.passed ? itemPassWord : itemFailWord}</span></td></tr>`,
					),
				)
				.join("")}
    </tbody>
  ...
  <h2>${sectionTitle("data.report.sectionMisra", "4. MISRA-C 合规性")}</h2>
  <table>
    <thead>
  ...
  <h2>${sectionTitle("data.report.sectionRepair", "5. 修复历史")}</h2>
  ...
  <h2>${sectionTitle("data.report.sectionSimulation", "6. 数字孪生仿真")}</h2>
  <p>仿真步数：${result.simulation_result.total_steps}，故障类型：${escapeHtml(result.simulation_result.fault_type ?? "无")}。</p>
  <p>结果：${simPassed ? `<span class="pass">${simResultPass}</span>` : `<span class="fail">${simResultFail}</span>`}</p>
  <p>${escapeHtml(summary.simulation_summary)}</p>

  <h2>${sectionTitle("data.report.sectionObjectives", "7. DO-178C 目标达成情况")}</h2>
