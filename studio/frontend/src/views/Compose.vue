...
// ====================================================================
// 契约模板库：从模板快速创建组件并填入 A / B 槽位
// ====================================================================

/** 模板分类 → 图标映射（lucide-vue-next 组件） */
...
const applyTemplate = (target: "A" | "B", template: ContractTemplate) => {
	if (target === "A") {
		compAName.value = template.name;
		compACode.value = template.code;
		compAContract.value = template.contractYaml;
	} else {
		compBName.value = template.name;
		compBCode.value = template.code;
		compBContract.value = template.contractYaml;
	}
...
<template>
  <div class="compose-page">
    ...
    <!-- 契约模板库 -->
    <Card class="template-library">
      <CardHeader>
        <CardTitle class="card-title">
          <Library class="title-icon" />
          契约模板库
          <span class="title-hint">
            点击「→ A」或「→ B」将模板快速填入对应组件槽位
          </span>
        </CardTitle>
      </CardHeader>
      <CardContent>
        <div class="template-grid">
          <div
            v-for="tpl in CONTRACT_TEMPLATES"
            :key="tpl.id"
            class="template-card"
            :class="{
              'applied-a': isTemplateAppliedTo('A', tpl),
              'applied-b': isTemplateAppliedTo('B', tpl),
            }"
          >
            <div class="tpl-header">
              <component
                :is="templateCategoryIcons[tpl.category] || Layers"
                class="tpl-icon"
              />
              ...
              <span class="tpl-safety" :title="`安全等级 ${tpl.safetyLevel}`">
                {{ tpl.safetyLevel }}
              </span>
            </div>
            ...
            <div class="tpl-signature">
              <div class="sig-row">
                <span class="sig-label">输入</span>
                <code class="sig-value">{{ formatSignals(tpl.inputs) }}</code>
              </div>
              <div class="sig-row">
                <span class="sig-label">输出</span>
                <code class="sig-value">{{ formatSignals(tpl.outputs) }}</code>
              </div>
              <div class="sig-row">
                <span class="sig-label">范围</span>
                <code class="sig-value">
                  {{ formatRange(tpl.outputs[0]?.range) }}
                </code>
              </div>
              <div class="sig-row">
                <span class="sig-label">约束</span>
                <span class="sig-value-text">
                  {{ tpl.invariants[0]?.description || '—' }}
                </span>
              </div>
            </div>
            ...
            <div class="tpl-actions">
              <Button
                size="sm"
                variant="outline"
                :class="{ active: isTemplateAppliedTo('A', tpl) }"
                title="将模板填入组件 A"
              ...
              <Button
                size="sm"
                variant="outline"
                :class="{ active: isTemplateAppliedTo('B', tpl) }"
                title="将模板填入组件 B"
...
/* ============ 契约模板库 ============ */
.template-library {
  border-top: 3px solid hsl(280, 60%, 55%);
}
...
.template-card:hover {
  ...
  box-shadow: 0 2px 8px hsla(280, 60%, 55%, 0.1);
}

.template-card.applied-a {
  border-color: hsl(220, 70%, 50%);
  box-shadow: 0 0 0 1px hsl(220, 70%, 50%);
}

.template-card.applied-b {
  border-color: #059669;
...
.tpl-actions :deep(button) {
  flex: 1;
  font-size: 12px;
}

.tpl-actions :deep(button.active) {
  border-color: hsl(280, 60%, 55%);
