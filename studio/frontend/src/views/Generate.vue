...
/** 需求追溯标签列表（来自追溯矩阵的 REQ-xxx，排序后展示为可点击徽章） */
const reqTags = computed<string[]>(() => {
	if (!result.value) return [];
	return Object.keys(result.value.traceability).sort();
});

/** 需求区点击 REQ 标签：切换激活并跳转到代码面板以展示高亮 */
const toggleReqTag = (req: string) => {
	activeTag.value = activeTag.value === req ? null : req;
	if (activeTag.value) focusedPanel.value = "code";
};
...
const onDownloadContractYaml = () => {
	if (!result.value) return;
	const name = (result.value.contract.component || "contract").replace(
		/[^A-Za-z0-9_]/g,
		"_",
	);
	downloadTextFile(
  ...
  <div class="generate-page">
    <header class="page-header">
      ...
      <button
        ...
        :title="highlightEnabled ? '关闭高亮追溯' : '开启高亮追溯：点击 Tag 联动高亮需求/代码/契约'"
        @click="highlightEnabled = !highlightEnabled"
      >
        <span class="highlight-toggle-dot" :class="{ on: highlightEnabled }" />
        高亮追溯：{{ highlightEnabled ? '开' : '关' }}
      </button>
    </header>
    ...
    <Card class="input-card">
      ...
      <CardContent>
        ...
        <!-- 需求追溯标签栏（双向追溯：点击 REQ 高亮代码，与代码区联动） -->
        <div v-if="status === 'done' && result && reqTags.length > 0" class="req-tags-bar">
          <span class="req-tags-label">🏷️ 需求追溯标签（点击高亮代码）：</span>
          <button
            v-for="req in reqTags"
            :key="req"
            type="button"
            class="req-tag-btn"
            :class="{ active: activeTag === req }"
            :title="`高亮 ${req} 关联代码行`"
            @click="toggleReqTag(req)"
          >[{{ req }}]</button>
          <button
            v-if="activeTag"
            type="button"
    ...
    <div v-if="status === 'done' && result" class="results-section">
      <Tabs v-model="activeTab" class="result-tabs">
        <TabsList class="tabs-list">
          ...
          <TabsTrigger value="verify">
            ...
            <span
              ...
              class="tab-badge"
              :class="{
                pass: verifyResult.status === 'passed',
                fail: verifyResult.status === 'failed',
              }"
            >{{
        ...
        <!-- Tab 5: 形式化验证（Task 5: Z3 SMT + CBMC） -->
        <TabsContent value="verify">
          <div class="verify-tab">
            <Card class="result-card verify-actions-card">
              <CardHeader>
                <CardTitle class="card-title">
                  🔬 形式化验证（Z3 SMT Solver + CBMC 有界模型检查）
                  <span class="title-hint">（数学证明契约一致性与代码安全性）</span>
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div class="verify-actions">
                  <Button :disabled="verifying" @click="onVerify">
                    <Loader2 v-if="verifying" class="animate-spin" />
                    <ShieldCheck v-else />
                    {{ verifyResult ? '重新验证' : '开始形式化验证' }}
                  </Button>
                  <span class="verify-hint">
                    将契约约束转换为 Z3 SMT 公式验证可满足性，并（可选）调用
                    CBMC 对生成的 C 代码进行有界模型检查。Z3/CBMC 缺失时
                    自动降级为 SKIPPED。
                  </span>
                </div>
              </CardContent>
            ...
            <FormalVerificationResult
              :result="verifyResult"
              :loading="verifying"
              @start-verify="onVerify"
            />
          </div>
        </TabsContent>

        <!-- Tab 6: DO-178C 报告（Day 4+） -->
