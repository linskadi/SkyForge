...
/**
 * HITLPanel 人机协作（Human-in-the-Loop）审批面板组件
 *
 * - 待审批列表（每项一个卡片）：
...
/** 检查点图标映射 */
const checkpointIconMap: Record<HITLCheckpointType, string> = {
	requirement_review: "📝",
	contract_review: "📋",
	code_review: "💻",
	final_review: "🏁",
...
/** 检查点颜色映射 */
const checkpointColorMap: Record<HITLCheckpointType, string> = {
...
/** 批准 */
const onApprove = async (item: HITLApproval) => {
...
onMounted(() => {
	loadPending();
	// 1s 更新 now.value 用于超时倒计时显示。页面不可见时暂停节省 CPU。
	timer = setInterval(() => {
		now.value = Date.now();
	}, 1000);
