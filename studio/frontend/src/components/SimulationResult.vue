...
.status-icon.spinning { color: hsl(var(--primary)); animation: spin 1s linear infinite; }
@keyframes spin { to { transform: rotate(360deg); } }
.status-title { font-size: 18px; font-weight: 700; color: hsl(var(--foreground)); }
.status-sub { font-size: 12px; color: hsl(var(--muted-foreground)); margin-top: 2px; }
...
.row-label { color: hsl(var(--muted-foreground)); flex-shrink: 0; min-width: 90px; }
.row-value { font-family: 'Consolas', monospace; color: hsl(var(--foreground)); word-break: break-all; }
...
.stat-card { display: flex; align-items: center; gap: 12px; padding: 12px 14px; background: hsl(var(--card)); border: 1px solid hsl(var(--border)); border-left: 3px solid; border-radius: 8px; }
.stat-icon { font-size: 22px; flex-shrink: 0; }
.stat-info { display: flex; flex-direction: column; }
.stat-label { font-size: 11px; color: hsl(var(--muted-foreground)); text-transform: uppercase; letter-spacing: 0.3px; }
