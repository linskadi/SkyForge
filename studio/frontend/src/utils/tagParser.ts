/**
 * 共享标签解析工具
 * 提取 [REQ-xxx] / [MISRA-Rule-x.x] / [CON-xxx-XXX-nnn] / [TST-xxx] 内联标签
 */
...
export type InlineToken =
	...
	| { type: "tst"; value: string };

/** 匹配 [REQ-xxx] / [MISRA-Rule-x.x] / [CON-xxx-XXX-nnn] / [TST-xxx] */
const TAG_REGEX = /\[(REQ-\d+|MISRA-Rule-[\d.]+|CON-\d+-[A-Z]+-\d+|TST-\d+)\]/g;
...
export function parseInlineTags(text: string): InlineToken[] {
	...
	while (true) {
		...
		} else if (tag.startsWith("TST-")) {
			tokens.push({ type: "tst", value: tag });
		}
